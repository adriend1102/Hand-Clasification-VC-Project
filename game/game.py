...
class Game:
    ...
    def render_camera(self):
        ...
        self.screen.blit(webcamScaled, webcamScaled.get_rect())

    def loop(self):
        #with self.cnn_model as self.cnn_model:
        while self.running:
    ...
    def process_camera(self):
        ...
        if image is not None:
            ...
            if results is not None:
                self.no_face = False
                if not self.mouthWasOpen and results%2 != 0:
                        pygame.event.post(pygame.event.Event(MOUTH_OPENED))
                        self.mouthWasOpen = True
                elif self.mouthWasOpen and results%2 == 0:
